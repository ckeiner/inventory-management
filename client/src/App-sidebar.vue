<template>
  <SaasLayout
    :logo="null"
    app-name="Inventory Manager"
    :user="currentUser"
    :nav-items="navItems"
    :quick-actions="quickActions"
    :search-enabled="true"
    @search="handleSearch"
  >
    <FilterBar />
    <main class="main-content">
      <router-view />
    </main>

    <ProfileDetailsModal
      :is-open="showProfileDetails"
      @close="showProfileDetails = false"
    />

    <TasksModal
      :is-open="showTasks"
      :tasks="tasks"
      @close="showTasks = false"
      @add-task="addTask"
      @delete-task="deleteTask"
      @toggle-task="toggleTask"
    />
  </SaasLayout>
</template>

<script>
import { ref, onMounted, computed } from 'vue'
import { api } from './api'
import { useAuth } from './composables/useAuth'
import { useI18n } from './composables/useI18n'
import { SaasLayout } from './components/saas-ui'
import FilterBar from './components/FilterBar.vue'
import ProfileDetailsModal from './components/ProfileDetailsModal.vue'
import TasksModal from './components/TasksModal.vue'
import { sidebarNavItems, quickActions } from './config/sidebar-nav'

export default {
  name: 'App',
  components: {
    SaasLayout,
    FilterBar,
    ProfileDetailsModal,
    TasksModal
  },
  setup() {
    const { currentUser } = useAuth()
    const { t } = useI18n()
    const showProfileDetails = ref(false)
    const showTasks = ref(false)
    const apiTasks = ref([])

    const navItems = ref(sidebarNavItems)

    // Merge mock tasks from currentUser with API tasks
    const tasks = computed(() => {
      return [...currentUser.value.tasks, ...apiTasks.value]
    })

    const loadTasks = async () => {
      try {
        apiTasks.value = await api.getTasks()
      } catch (err) {
        console.error('Failed to load tasks:', err)
      }
    }

    const addTask = async (taskData) => {
      try {
        const newTask = await api.createTask(taskData)
        apiTasks.value.unshift(newTask)
      } catch (err) {
        console.error('Failed to add task:', err)
      }
    }

    const deleteTask = async (taskId) => {
      try {
        const isMockTask = currentUser.value.tasks.some(t => t.id === taskId)

        if (isMockTask) {
          const index = currentUser.value.tasks.findIndex(t => t.id === taskId)
          if (index !== -1) {
            currentUser.value.tasks.splice(index, 1)
          }
        } else {
          await api.deleteTask(taskId)
          apiTasks.value = apiTasks.value.filter(t => t.id !== taskId)
        }
      } catch (err) {
        console.error('Failed to delete task:', err)
      }
    }

    const toggleTask = async (taskId) => {
      try {
        const mockTask = currentUser.value.tasks.find(t => t.id === taskId)

        if (mockTask) {
          mockTask.status = mockTask.status === 'pending' ? 'completed' : 'pending'
        } else {
          const updatedTask = await api.toggleTask(taskId)
          const index = apiTasks.value.findIndex(t => t.id === taskId)
          if (index !== -1) {
            apiTasks.value[index] = updatedTask
          }
        }
      } catch (err) {
        console.error('Failed to toggle task:', err)
      }
    }

    const handleSearch = (query) => {
      console.log('Search query:', query)
      // Can be extended with actual search functionality
    }

    onMounted(loadTasks)

    return {
      t,
      currentUser,
      navItems,
      quickActions,
      showProfileDetails,
      showTasks,
      tasks,
      addTask,
      deleteTask,
      toggleTask,
      handleSearch
    }
  }
}
</script>

<style>
* {
  margin: 0;
  padding: 0;
  box-sizing: border-box;
}

body {
  font-family: 'Inter', -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, Oxygen, Ubuntu, Cantarell, sans-serif;
  background: var(--saas-content-bg);
  color: var(--saas-text-primary);
  -webkit-font-smoothing: antialiased;
  -moz-osx-font-smoothing: grayscale;
}

.main-content {
  padding: 1.5rem 2rem;
}

.page-header {
  margin-bottom: 1.5rem;
}

.page-header h2 {
  font-size: 1.875rem;
  font-weight: 700;
  color: var(--saas-text-primary);
  margin-bottom: 0.375rem;
  letter-spacing: -0.025em;
}

.page-header p {
  color: var(--saas-text-muted);
  font-size: 0.938rem;
}

.stats-grid {
  display: grid;
  grid-template-columns: repeat(auto-fit, minmax(280px, 1fr));
  gap: 1.25rem;
  margin-bottom: 1.5rem;
}

.stat-card {
  background: var(--saas-card-bg);
  padding: 1.25rem;
  border-radius: var(--saas-card-radius);
  border: 1px solid var(--saas-card-border);
  transition: all 0.2s ease;
}

.stat-card:hover {
  border-color: var(--saas-border-dark);
  box-shadow: var(--saas-card-shadow-hover);
}

.stat-label {
  color: var(--saas-text-secondary);
  font-size: 0.875rem;
  font-weight: 600;
  text-transform: uppercase;
  letter-spacing: 0.5px;
  margin-bottom: 0.625rem;
}

.stat-value {
  font-size: 2.25rem;
  font-weight: 700;
  color: var(--saas-text-primary);
  letter-spacing: -0.025em;
}

.stat-card.warning .stat-value {
  color: var(--saas-warning);
}

.stat-card.success .stat-value {
  color: var(--saas-success);
}

.stat-card.danger .stat-value {
  color: var(--saas-error);
}

.stat-card.info .stat-value {
  color: var(--saas-info);
}

.card {
  background: var(--saas-card-bg);
  border-radius: var(--saas-card-radius);
  padding: var(--saas-card-padding);
  border: 1px solid var(--saas-card-border);
  margin-bottom: 1.25rem;
}

.card-header {
  display: flex;
  justify-content: space-between;
  align-items: center;
  margin-bottom: 1rem;
  padding-bottom: 0.875rem;
  border-bottom: 1px solid var(--saas-card-border);
}

.card-title {
  font-size: 1.125rem;
  font-weight: 700;
  color: var(--saas-text-primary);
  letter-spacing: -0.025em;
}

.table-container {
  overflow-x: auto;
}

table {
  width: 100%;
  border-collapse: collapse;
}

thead {
  background: var(--saas-content-bg-alt);
  border-top: 1px solid var(--saas-card-border);
  border-bottom: 1px solid var(--saas-card-border);
}

th {
  text-align: left;
  padding: 0.5rem 0.75rem;
  font-weight: 600;
  color: var(--saas-text-secondary);
  font-size: 0.75rem;
  text-transform: uppercase;
  letter-spacing: 0.05em;
}

td {
  padding: 0.5rem 0.75rem;
  border-top: 1px solid var(--saas-content-bg-alt);
  color: var(--saas-text-secondary);
  font-size: 0.875rem;
}

tbody tr {
  transition: background-color 0.15s ease;
}

tbody tr:hover {
  background: var(--saas-content-bg-alt);
}

.badge {
  display: inline-block;
  padding: 0.313rem 0.75rem;
  border-radius: var(--saas-radius-sm);
  font-size: 0.75rem;
  font-weight: 600;
  text-transform: uppercase;
  letter-spacing: 0.025em;
}

.badge.success {
  background: var(--saas-success-light);
  color: #065f46;
}

.badge.warning {
  background: var(--saas-warning-light);
  color: #92400e;
}

.badge.danger {
  background: var(--saas-error-light);
  color: #991b1b;
}

.badge.info {
  background: var(--saas-info-light);
  color: #1e40af;
}

.badge.increasing {
  background: var(--saas-success-light);
  color: #065f46;
}

.badge.decreasing {
  background: var(--saas-error-light);
  color: #991b1b;
}

.badge.stable {
  background: var(--saas-info-light);
  color: #3730a3;
}

.badge.high {
  background: var(--saas-error-light);
  color: #991b1b;
}

.badge.medium {
  background: var(--saas-warning-light);
  color: #92400e;
}

.badge.low {
  background: var(--saas-info-light);
  color: #1e40af;
}

.loading {
  text-align: center;
  padding: 3rem;
  color: var(--saas-text-muted);
  font-size: 0.938rem;
}

.error {
  background: var(--saas-error-light);
  border: 1px solid var(--saas-error);
  color: #991b1b;
  padding: 1rem;
  border-radius: var(--saas-radius-md);
  margin: 1rem 0;
  font-size: 0.938rem;
}
</style>
